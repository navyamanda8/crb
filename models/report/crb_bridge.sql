...
product_churn_lm AS (
    ...
    next_month_usage AS (
        SELECT 
            customer_id, 
            product_id, 
            DATE_TRUNC('month', payment_month - INTERVAL '1 month') AS prev_month
        FROM product_usage
    )
    ...
    SELECT 
        pu.payment_month,
        pu.product_id,
        COUNT(DISTINCT pu.customer_id) AS churned_customers_lm
    FROM product_usage pu
    LEFT JOIN next_month_usage nm
        ON pu.customer_id = nm.customer_id
        AND pu.product_id = nm.product_id
        AND pu.payment_month = nm.prev_month
    WHERE nm.customer_id IS NULL
    GROUP BY pu.payment_month, pu.product_id
),
